...
export default function FilterBar({ filters, onChange }) {
  ...
  const applyPreset = (p) => {
    ...
    if (p === "this_month") onChange({ ...filters, startDate: firstOfMonth(), endDate: todayStr() });
    if (p === "last_month") onChange({ ...filters, startDate: firstOfLastMonth(), endDate: lastOfLastMonth() });
  };
    ...
    <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm">
      <div className="flex flex-wrap gap-3 items-end">
        ...
        <div className="flex flex-col gap-1 min-w-32.5">
          ...
          <Select value={filters.category ?? "All"} onChange={(v) => onChange({ ...filters, category: v })}>
            <option>All</option>
            ...
            {PRESETS.map((p) => (
              <button key={p.value} onClick={() => applyPreset(p.value)}
                className={`px-3 py-2 rounded-xl text-xs font-medium transition-colors ${
                  preset === p.value ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                }`}>
                {p.label}
              </button>
            ))}
        ...
        {preset === "custom" && (
          <div className="flex gap-2 items-end flex-wrap">
            <div className="flex flex-col gap-1">
              ...
              <input type="date" max={todayStr()} value={filters.startDate ?? ""}
                onChange={(e) => onChange({ ...filters, startDate: e.target.value })}
                className="border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-xs font-medium text-gray-400">To</label>
              <input type="date" max={todayStr()} value={filters.endDate ?? ""}
        ...
        <div className="flex gap-2 ml-auto items-end">
          <div className="flex flex-col gap-1 min-w-30">
            ...
            <Select value={filters.sortBy ?? "date"} onChange={(v) => onChange({ ...filters, sortBy: v })}>
          ...
          <a
            href={csvDownloadUrl({ category: filters.category, startDate: filters.startDate, endDate: filters.endDate })}
            download
